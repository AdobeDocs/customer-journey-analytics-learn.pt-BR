---
title: Adicionar visualizações de [!UICONTROL Dispersão] em [!DNL Analysis Workspace] Projetos
description: Saiba como usar a visualização [!UICONTROL dispersão] em [!DNL Analysis Workspace] projetos no [!DNL Customer Journey Analytics].
feature: Visualizations
kt: 13428
role: User
level: Beginner
last-substantial-update: 2026-02-17T00:00:00Z
exl-id: 2733a815-c030-4fad-81d9-3f38fb1e7378
source-git-commit: 6184b0e86b72026dd95e9c31ac13ce674bc678f1
workflow-type: tm+mt
source-wordcount: '54'
ht-degree: 0%

---

# Adicionar visualizações de [!UICONTROL dispersão] em [!DNL Analysis Workspace] projetos em [!DNL Customer Journey Analytics]

Saiba como usar a visualização [!UICONTROL dispersão] em projetos [!DNL Analysis Workspace] no [!DNL Customer Journey Analytics]. Essa visualização mostra a relação entre itens de dimensão e até três métricas.

>[!VIDEO](https://video.tv.adobe.com/v/3479974/?captions=por_br&quality=12&learn=on)

Para obter mais informações sobre as visualizações de [!UICONTROL dispersão], visite a [documentação](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-workspace/visualizations/scatterplot.html?lang=pt-BR){target="_blank"}.
